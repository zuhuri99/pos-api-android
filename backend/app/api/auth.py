...
def login(payload: LoginRequest, db: Session = Depends(get_db)):
    ...
    if payload.device_type == "web" and not settings.web_frontend_enabled:
        raise HTTPException(403, "Login melalui web dinonaktifkan.")
    user = db.scalar(select(User).where(User.username == payload.username))
    if not user or not user.is_active or not verify_password(payload.password, user.password_hash):
        raise HTTPException(401, "Username atau password salah.")
    raw_token = new_token()
    ttl_days = settings.admin_token_ttl_days if user.is_admin else settings.user_token_ttl_days
