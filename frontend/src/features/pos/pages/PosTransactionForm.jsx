      ...
      <form onSubmit={requestSubmit} className="mx-auto w-full min-w-0 max-w-[1600px] overflow-x-clip pb-28 lg:pb-4">
        ...
        <div className="grid min-w-0 gap-3 lg:grid-cols-[minmax(0,1.5fr)_minmax(320px,0.72fr)] xl:grid-cols-[minmax(0,1.65fr)_400px]">
          ...
          <aside className="relative z-10 h-fit min-w-0 rounded-[24px] border border-white/80 bg-white/80 p-3 shadow-[0_18px_50px_rgba(15,23,42,0.10)] backdrop-blur-2xl sm:p-4 lg:sticky lg:top-0">
            ...
            <button type="button" onClick={() => setShowMore((value) => !value)} className={`mt-4 flex w-full items-center justify-center gap-2 rounded-xl border px-3 py-3 text-xs font-black shadow-sm transition ${showMore || asNumber(form.discount_amount) > 0 || form.sale_note ? "border-orange-400 bg-gradient-to-r from-amber-200 to-orange-200 text-orange-950 ring-2 ring-orange-100" : "border-amber-300 bg-gradient-to-r from-amber-50 to-orange-100 text-amber-900"}`}><span aria-hidden="true">✦</span>{showMore ? "Sembunyikan Diskon & Catatan" : "Diskon & Catatan"}<span aria-hidden="true">›</span></button>
            {showMore && <div className="mt-3 grid grid-cols-2 gap-2 rounded-2xl border border-amber-200 bg-amber-50 p-3"><select value={form.discount_type} onChange={(event) => setForm((current) => ({ ...current, discount_type: event.target.value }))} className="min-w-0 rounded-xl border border-amber-300 bg-white px-2 py-2 text-xs"><option value="fixed">Diskon nominal</option><option value="percentage">Diskon persen</option></select><input type="text" inputMode="decimal" value={formatNumberInput(form.discount_amount)} onFocus={(event) => event.target.select()} onChange={(event) => setForm((current) => ({ ...current, discount_amount: parseFormattedNumber(event.target.value) }))} className="min-w-0 rounded-xl border border-amber-300 px-2 py-2 text-xs" placeholder="Diskon" /><textarea value={form.sale_note || ""} onChange={(event) => setForm((current) => ({ ...current, sale_note: event.target.value }))} className="col-span-2 rounded-xl border border-amber-300 px-3 py-2 text-xs" rows="2" placeholder="Catatan transaksi" /></div>}
