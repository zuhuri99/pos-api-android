      ...
      <form onSubmit={requestSubmit} className="mx-auto w-full min-w-0 max-w-[1600px] overflow-x-clip pb-28 lg:pb-4">
        ...
        <div className="grid min-w-0 gap-3 lg:grid-cols-[minmax(0,1.5fr)_minmax(320px,0.72fr)] xl:grid-cols-[minmax(0,1.65fr)_400px]">
          ...
          <aside className="relative z-10 h-fit min-w-0 rounded-[24px] border border-white/80 bg-white/80 p-3 shadow-[0_18px_50px_rgba(15,23,42,0.10)] backdrop-blur-2xl sm:p-4 lg:sticky lg:top-0">
            ...
            <button type="button" onClick={() => setShowMore((value) => !value)} className={`mt-4 flex w-full items-center justify-center gap-1.5 rounded-xl border px-3 py-3 text-xs font-black shadow-sm transition ${showMore || asNumber(form.discount_amount) > 0 || form.sale_note ? "border-orange-400 bg-gradient-to-r from-amber-200 to-orange-200 text-orange-950 ring-2 ring-orange-100" : "border-amber-300 bg-gradient-to-r from-amber-50 to-orange-100 text-amber-900"}`}>{showMore ? "Sembunyikan Diskon & Catatan" : "Diskon & Catatan"}<span aria-hidden="true" className="text-[10px]">▾</span></button>
